...
\`\`\`
status-data/
├── current.json           # Latest readings from all systems
├── incidents.json         # Incident history
├── maintenance.json       # Scheduled maintenance windows
└── archives/
    └── YYYY/MM/           # Historical data organized by year/month
        └── system-name/   # Per-system archives
\`\`\`
...
async function setupOrphanedBranch(force = false) {
  console.log('🚀 Setting up orphaned status-data branch...\n');
  ...
  if (force && (localExists || remoteExists)) {
    console.log(`🗑️  Deleting existing '${BRANCH_NAME}' branch...`);
    if (localExists) {
  ...
  console.log(`📝 Creating orphaned branch '${BRANCH_NAME}'...`);
  exec(`git checkout --orphan ${BRANCH_NAME}`);
  console.log(`   ✅ Branch created\n`);

  // Step 5: Remove all files from staging
  console.log(`🧹 Cleaning staging area...`);
  try {
    exec('git rm -rf .');
  } catch (error) {
...
`;
  ...
  console.log(`✨ Success! Orphaned branch '${BRANCH_NAME}' is ready.\n`);
