    ...
    steps:
      ...
      - name: Commit status data
        ...
        run: |
          ...
          git add .
          git commit -m "📊 Status update: $(date -u +%Y-%m-%dT%H:%M:%SZ)" || echo "No changes"
...
|--------|-------------|-----------------|--------|
| Repo size | Grows indefinitely | Isolated growth | ✅ Orphaned |
| Clone speed | Slows over time | Main stays fast | ✅ Orphaned |
| History clarity | Polluted with status commits | Clean code history | ✅ Orphaned |
| Setup complexity | Simple | Requires initial setup | ⚖️ Main |
| CI/CD isolation | May trigger unnecessarily | Separate triggers | ✅ Orphaned |
| Data retention | Hard to prune | Easy to truncate | ✅ Orphaned |
| Industry standard | Not common | Used by Upptime | ✅ Orphaned |
